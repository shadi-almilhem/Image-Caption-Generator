    ...
    <div className={`flex flex-wrap gap-2 ${className}`}>
      {options.map((option: any) => (
        <button
          ...
          className={`px-3 py-2 shadow-[inset_0px_-1px_0px_0px_rgba(0,0,0,0.1),inset_0px_1px_0px_0px_rgba(255,255,255,0.3)]  
  rounded-full text-sm font-medium transition-colors ${
    (multiple ? value.includes(option) : value === option)
      ? "bg-linear-to-b from-purple-500 to-purple-600 text-white"
